...
export class BackendService {
  ...
  createDraftFromDataset(dmp: Dmp, dataset: Dataset): Observable<Dmp> {
    const httpOptions = {
      headers: new HttpHeaders({
        'Content-Type': 'application/json',
      }),
    ...
    return this.http
      .post<Dmp>(
        `${this.backendUrl}invenio-damap/dmps/${dmp.id}/dataset/${dataset.id}`,
        null,
        httpOptions
      )
      .pipe(retry(0), catchError(this.handleError('http.error.plans.update')));
  }
